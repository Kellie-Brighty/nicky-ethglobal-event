...
pub trait IRestaurantManager<TContractState> {
    fn add_restaurant(
        ref self: TContractState, name: felt252, location: felt252, owner: ContractAddress, image_url: felt252
    );
    fn get_restaurant(
        self: @TContractState, owner: ContractAddress,
    ) -> Option<(felt252, felt252, ContractAddress)>;

    fn get_all_restaurants(self: @TContractState) -> Array<(felt252, felt252, ContractAddress, felt252)>;
...
mod RestaurantManager {
    ...
    impl RestaurantManagerImpl of super::IRestaurantManager<ContractState> {
        fn add_restaurant(
            ref self: ContractState, name: felt252, location: felt252, owner: ContractAddress, image_url: felt252,
        ) {
            let id = self.next_restaurant_id.read();
            self.next_restaurant_id.write(id + 1);
            let new_restaurant = Restaurant { id, name, location, owner, image_url };
        ...
        fn get_all_restaurants(self: @ContractState) -> Array<(felt252, felt252, ContractAddress, felt252)> {
            ...
            loop {
                ...
                let restaurant = self.restaurants.at(i).read();
                result.append((restaurant.name, restaurant.location, restaurant.owner, restaurant.image_url));
                i += 1;
            };
            result
        }
        ...
        fn get_all_menu_items(self: @ContractState) -> Array<(felt252, felt252, u64, felt252, u64)> {
        let mut menu_items = ArrayTrait::new();
        let mut i: u64 = 0;
        loop {
            if i >= self.menu_items.len() {
